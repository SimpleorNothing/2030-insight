...
| **S2 트렌드 도출** | 무엇이 뜨나 | 선정기준·탈락사유 → 9대 메가트렌드 카드(T1~T9) · 제품 카테고리 레벨 히트맵 |
...
자동·주기적 피드가 아니라, **사용자가 버튼을 눌렀을 때만** Gemini를 호출하는 온디맨드 방식이다.

**흐름:** 소스 선택 → `Insight 뽑기` → Gemini가 신호 도출·S1~S6 분류 → 반영할 항목 선택 → 1~6단계 분석 테이블에 반영(+신규반영 배지)

- **모델:** `gemini-3.6-flash`
- **프록시:** `https://da-insight-anthropic-proxy.cw120-park.workers.dev`
  - Gemini 키는 워커 secret에 보관 — 브라우저로 노출되지 않음 (`market-insight`와 동일 인프라)
- **소스 4종(MI·2030·agentguide·추가)** 은 *실데이터 fetch가 아니라* 선택 시 해당 **라벨 텍스트가 Gemini 프롬프트 맥락으로 전달**되는 방식이다. (추가 소스만 URL·파일·텍스트 직접 입력)
- **폴백:** 프록시 미연결 시 하드코딩 샘플(`DEMO`, 9건)로 흐름을 시연하며 "백엔드 미연결 — 샘플 Insight" 배너를 표시
- **반영 결과:** 선택한 신호는 해당 STEP의 트렌드별(T1~T9) 분석 테이블로 들어가고, 기존 9대 트렌드 카드(S2)·경쟁사 히트맵(S4) 행에 신규반영 배지(`+N ▲/▼`)로 연동된다. `×`로 반영 취소 가능
...
경쟁사 전략 추적 보드(**CI**, `ci.samsungda.net`)의 **편집국 톤을 전면 적용**한다
(STYLE_GUIDE §8-2 자체 스타일 레지스트리 기준). 과거 Pantone Blue Fusion 독자 팔레트에서
명시 요청으로 CI 트랙으로 전환했다. **색 팔레트에 더해 각진 모서리(radius 0)와 카드 좌측
액센트 바까지** CI 보드와 시각적으로 일치시켰다 — 색만 교체하는 §8-3 원칙을 벗어난
**명시 예외**다(스피너 등 기능상 원형은 유지). 타이포그래피는 Pretendard.

| 토큰 | 값 | 용도 |
|------|------|------|
| `--paper` | `#EDEFEC` | 페이지 배경 |
| `--surface` | `#FFFFFF` | 카드·표 배경(패널) |
| `--ink` | `#17222D` | 본문 텍스트 |
| `--muted` | `#5C6B79` | 보조 설명 |
| `--line` / `--line-strong` | `#D3D9D6` / `#9AA6A0` | 구분선·테두리 |
| `--accent` | `#46647E` (new) | 강조·액티브·헤더 |
| `--signal` | `#A9790F` (amber) | 미검증·주의 신호 (이탤릭) |
| `--ok` | `#2F614D` (deep) | 선도·반영 완료 |
